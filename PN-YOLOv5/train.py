...
saining:
    ...
    $pip install setuptools==50.0.0 
    CUDA_VISIBLE_DEVICES=2 python /home/zfj/zzh/yolov5-master/train.py --data /home/zfj/zzh/yolov5-master/data/image_wrjall_qiushao.yaml --weights yolov5-master/yolov5l6_used_to_exp.pt --cfg /home/zfj/zzh/yolov5-master/models/yolov5pro_s1.yaml --cfg /home/zfj/zzh/yolov5-bingchong/models/yolov5l.yaml --img 2000 --batch-size 8 --device 2,3 --noval

   $ CUDA_VISIBLE_DEVICES=2 nohup python /home/zfj/zzh/yolov5-master/train.py --data /home/zfj/zzh/yolov5-master/data/image_wrjall_qiushao.yaml --weights yolov5-master/yolov5l6_used_to_exp.pt  --cfg /home/zfj/zzh/yolov5-bingchong/models/yolov5l.yaml --img 1000 --batch-size 14 --device 2,3 --noval > output_pretrain.log 2>&1 &
...
def train(hyp, opt, device, callbacks):
    ...
    if hasattr(dataset, 'labels') and dataset.labels:  # 先判断有没有 labels 属性，再判断是否为空
        model.class_weights = labels_to_class_weights(dataset.labels, nc).to(device) * nc
    else:
        model.class_weights = torch.ones(nc, device=device)  # 如果没有 labels，就用全 1 作为默认值
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, paths) in pbar:  # batch -------------------------------------------------------------
            # print(imgs.shape)
            # print(paths)
            # input("")
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                loss = compute_loss(pred,paths)  # loss scaled by batch_size
                if RANK != -1:
        ...
        # 计算 epoch 级别的平均 loss
        avg_loss = mloss2 / nb  # nb 是 batch 总数
        ...
        if RANK in {-1, 0}:
            ...
            # 根据 avg_loss 是否创历史新低来保存模型参数
            if avg_loss < best_loss:
                best_loss = avg_loss
                ckpt = {
                    "epoch": epoch,
                    "avg_loss": avg_loss,  # 记录当前 avg_loss
                ...
                save_path = "/home/zfj/zzh/yolov5-master/best_model/model.pt"
                torch.save(ckpt, save_path)
                callbacks.run("on_model_save", save_path, epoch, final_epoch, best_fitness, fi)
                print("已保存",save_path)
