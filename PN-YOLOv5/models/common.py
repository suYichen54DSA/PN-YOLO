...
class Prototype(nn.Module):
    def __init__(self, num_classes, feat_dim, momentum=0.9):
        ...
        # 初始化类别原型，并注册为 buffer
        # 使用 register_buffer 可以在保存和加载模型时保留类别原型
        self.register_buffer('prototypes', torch.zeros(num_classes, feat_dim))
        self.register_buffer('class_count', torch.zeros(num_classes))  # 记录每个类别的样本数量
        
    def forward(self, x, labels=None):
        loss=0
        # 如果 labels 为 None，则继续更新
        if labels is None:
            ...
            # 将空间维度展开，并转为 [batch_size * h * w, channels]
            x = x.permute(0, 2, 3, 1).reshape(-1, channels)
            x = F.normalize(x, dim=1)
            # 计算相似度矩阵
            similarity_matrix = torch.matmul(x, self.prototypes.t())
            positive_similarity, _ = similarity_matrix.max(dim=1)
            loss = (1 - positive_similarity).mean()
            isPro=True
        return loss,isPro
    ...
    def update_prototypes(self, features):
        ...
        self.prototypes = self.momentum * self.prototypes + (1 - self.momentum) * global_center
