...
def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=False, scaleFill=True, scaleup=True, stride=32):
    """强制所有图片缩放到 `new_shape`，避免 batch 里图片尺寸不一致"""
    ...
    # 计算缩放比例
    if scaleFill:  # 直接拉伸到目标尺寸
        new_unpad = (new_shape[1], new_shape[0])  # 直接使用目标尺寸
        ratio = new_shape[1] / shape[1], new_shape[0] / shape[0]  # 宽高缩放比
        dw, dh = 0, 0  # 不进行填充
    else:
        ...
        dw, dh = dw / 2, dh / 2  # 填充分配到两侧

    # 统一 resize 操作
    im = cv2.resize(im, new_unpad, interpolation=cv2.INTER_LINEAR)

    # 计算填充（如果 `scaleFill=False`，才需要填充）
